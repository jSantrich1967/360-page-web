...
export async function publishToMeta(
  ...
  try {
    ...
    const nextRetryDelay = RETRY_DELAYS[newRetries - 1] ?? RETRY_DELAYS[RETRY_DELAYS.length - 1];
    ...
    await supabaseAdmin.from('audit_logs').insert({
      ...
      entity_id: job.id,
      new_data: { error: error.message, platform: job.platform, retries: newRetries },
    });
  }
}
...
async function publishInstagramReel(
  ...
  await waitForMediaReady(igAccountId, mediaId, token, 60, 10000);
